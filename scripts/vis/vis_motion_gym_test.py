...
def setup(cfg):
    ...
    motion_lib = MotionLibReal(motion_file=cfg, device=device, fix_height=False, multi_thread=False)
    num_motions = 30
    curr_start = 0
...
def main(cfg : DictConfig) -> None:
    robot = cfg.robot
    xml_path = robot.
    motion_file = "../data_process/data/kpl/v1/amass_all.pkl"
    

motion_file = "../data_process/data/kpl/v1/amass_all.pkl"
xml_path = "/home/user/lhl/PHC/phc/data/assets/robot/hust_kpl_new/1230URDF.xml"
xml_path_2 = "phc/data/assets/robot/hust_kpl_new/1230URDF.xml"

asset_descriptors = [
    AssetDesc(xml_path, False),
]
...
motion_lib = MotionLibKpl(motion_file=motion_file, device=device, masterfoot_conifg=None, fix_height=False, multi_thread=False)
num_motions = 1
curr_start = 0
motion_lib.load_motions(skeleton_trees=[sk_tree] * num_motions, gender_betas=[torch.zeros(17)] * num_motions, limb_weights=[np.zeros(10)] * num_motions, random_sample=False)
motion_keys = motion_lib.curr_motion_keys
...
while not gym.query_viewer_has_closed(viewer):
    ...
    if args.show_axis:
        gym.clear_lines(viewer)
    idx = 0
    for pos_joint in rb_pos[0, 1:]: # idx 0 base (duplicate with 11)
